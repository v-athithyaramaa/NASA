...
const CrewInfo = ({ crewData = [] }) => {
  ...
  useEffect(() => {
    const fetchAstronauts = async () => {
      try {
        ...
        // ✅ Fetch via backend HTTPS proxy
        const response = await fetch("/astronauts");
        const data = await response.json();

        // Keep Open Notify reference
        console.log(
          "Fetched real-time astronaut data from Open Notify API via backend proxy:",
          data
        );
  ...
  const enhancedCrewData = sourceCrewData.map((member, index) => {
    ...
    const getCrewDetails = (astronautName) => {
      const nameMap = {
        "Oleg Kononenko": { country: "🇷🇺 Russia", agency: "Roscosmos", role: "Commander" },
        "Nikolai Chub": { country: "🇷🇺 Russia", agency: "Roscosmos", role: "Flight Engineer" },
        "Tracy Caldwell Dyson": { country: "🇺🇸 USA", agency: "NASA", role: "Flight Engineer" },
        "Matthew Dominick": { country: "🇺🇸 USA", agency: "NASA", role: "Commander" },
        "Michael Barratt": { country: "🇺🇸 USA", agency: "NASA", role: "Flight Engineer" },
        "Jeanette Epps": { country: "🇺🇸 USA", agency: "NASA", role: "Flight Engineer" },
        "Alexander Grebenkin": { country: "🇷🇺 Russia", agency: "Roscosmos", role: "Flight Engineer" },
        "Butch Wilmore": { country: "🇺🇸 USA", agency: "NASA", role: "Pilot" },
        "Sunita Williams": { country: "🇺🇸 USA", agency: "NASA", role: "Commander" },
      };
      return (
        nameMap[astronautName] || { country: "🌍 International", agency: "Space Agency", role: "Flight Engineer" }
      );
    };

    const details = getCrewDetails(name);

    const expertiseList = ["space medicine", "robotics", "EVA operations", "life sciences", "orbital mechanics", "Earth observation"];
    const experimentsList = ["Protein Crystal Growth", "Fluid Physics Experiments", "Human Research Program", "Earth Observation Studies", "Technology Demonstrations", "Plant Growth Experiments"];
    ...
    return {
      ...
      expeditionNumber: 72,
      arrivalDate: moment().subtract(Math.random() * 180, "days").format("YYYY-MM-DD"),
      plannedDeparture: moment().add(Math.random() * 180, "days").format("YYYY-MM-DD"),
      ...
      bio: `Experienced astronaut currently serving aboard the ISS with expertise in ${expertiseList[index % expertiseList.length]} and space operations.`,
      ...
      >
        ...
        >
          ...
          <div className="crew-basic-info">
            ...
            <div className="crew-expedition">Expedition {member.expeditionNumber}</div>
          </div>
            ...
            <div className="details-content">
              <div className="detail-section">
                ...
                <div className="timeline-item">
                  ...
                  <span className="timeline-value">{moment(member.arrivalDate).format("MMM DD, YYYY")}</span>
                </div>
                <div className="timeline-item">
                  <span className="timeline-label">Planned Departure:</span>
                  <span className="timeline-value">{moment(member.plannedDeparture).format("MMM DD, YYYY")}</span>
                </div>
                <div className="timeline-item">
                  <span className="timeline-label">Total Space Time:</span>
                  <span className="timeline-value">{member.totalSpaceTime} days</span>
                </div>
              </div>
              <div className="detail-section">
                ...
                <div className="experiments-list">
                  {member.experiments.map((experiment, idx) => (
                    <div key={idx} className="experiment-tag">{experiment}</div>
                  ))}
                </div>
              </div>
              <div className="detail-section">
  ...
  const CrewSchedule = () => {
    const scheduleEvents = [
      { time: "06:00 GMT", activity: "Daily Planning Conference", participants: "All Crew", type: "planning" },
      { time: "08:30 GMT", activity: "Protein Crystal Growth Experiment", participants: enhancedCrewData[0]?.name || "Crew Member 1", type: "science" },
      { time: "11:00 GMT", activity: "EVA Preparation", participants: "EVA Team", type: "eva" },
      { time: "14:00 GMT", activity: "Exercise Session", participants: "All Crew", type: "exercise" },
      { time: "16:30 GMT", activity: "Earth Observation Session", participants: enhancedCrewData[1]?.name || "Crew Member 2", type: "science" },
      { time: "19:00 GMT", activity: "Evening Planning Conference", participants: "All Crew", type: "planning" },
    ];
      ...
      <div className="crew-schedule">
        ...
        <div className="schedule-list">
          {scheduleEvents.map((event, index) => (
            <motion.div key={index} className={`schedule-item type-${event.type}`}
            ...
            >
              ...
              <div className="schedule-activity">
                ...
                <div className="activity-details">
                  ...
                  <div className="activity-participants">{event.participants}</div>
                </div>
              </div>
            </motion.div>
          ))}
  ...
  if (!sourceCrewData || sourceCrewData.length === 0) return (
    <div className="crew-info-loading">
      <Users size={48} />
    ...
    <div className="crew-info-container">
      <div className="api-status">
        <div className="status-badge success">
          <Globe size={16} />
          Live Data: {realCrewData.length} astronauts currently in space (via Open Notify API)
        </div>
      </div>
      ...
      <div className="crew-section">
        ...
        <p className="crew-subtitle">
          Real-time data from <strong>Open Notify API</strong> (via secure backend)
        </p>
        <div className="crew-members-list">
          {enhancedCrewData.map((member, index) => (
            <CrewMemberCard key={`${member.name}-${index}`} member={member} index={index} />
          ))}
        </div>
      </div>
      ...
      <div className="crew-footer">
        <p>
          Real-time crew data from <strong>Open Notify API</strong> • Mission details from NASA and international space agencies • Updated automatically on page load
        </p>
      </div>
    </div>
  );
